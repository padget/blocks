#ifndef __blocks_i18n_fr_h__
#define __blocks_i18n_fr_h__

#ifdef BLOCKS_LOCAL_LANG_FR

#define blocks_log_memory_allocation "la mémoire ne peut pas être allouée : %s"

#define blocks_log_argument_already_defined "the arguments has already been defined"
#define blocks_log_argument_must_be_number "l'argument %s : %s doit être un nombre"
#define blocks_log_argument_file_no_value "--file doit avoir une valeur"
#define blocks_log_argument_file_mandatory "--file est obligatoire"

#define blocks_log_empty_filename "le nom du fichier ne doit pas être vide"
#define blocks_log_file_not_found "fichier non trouvé : %s"
#define blocks_log_no_content_in_file "rien à lire dans le fichier : %s"
#define blocks_log_file_null_pointer "pointeur de fichier NULL"

#define blocks_log_no_command "aucune commande à lire dans la source"


#endif

#endif